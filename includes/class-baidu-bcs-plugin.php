...
class Baidu_BCS_Plugin {
	...
	function __construct($plugin_file) {
		...
		if ($this->bucket_name) {
			add_filter ( 'option_upload_url_path', array (
					$this,
					'option_upload_url_path' 
			) );
		}
		...
		if (is_admin ()) {
			...
			if ($this->bucket_name) {
				...
				add_filter ( 'wp_save_image_editor_file', array (
						$this,
...
<?php
	 ...
	 */
	function admin_menu() {
		add_options_page ( '百度云存储设置', '百度云存储', 'manage_options', $this->plugin_name, array (
				$this,
	...
	function register_settings() {
		register_setting ( self::OPTION_GROUP, self::OPTION_BUCKET_NAME, array (
				$this,
				'vaildate_bucket_name' 
		) );
	}
	...
	function upload_file_to_bcs($file) {
		...
		$res = $this->get_BaiduBCS ()->create_object ( $this->bucket_name, $object, $file, array (
